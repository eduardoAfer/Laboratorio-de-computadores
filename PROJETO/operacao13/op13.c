...
void ppm_overlay(float alfa, PPM_Image overlay, PPM_Image image, FILE *output, int y, int x){
    ...
    for(int iRes=x, iOvr=0; iRes<imgRes.lines && iOvr<overlay.lines; iRes++, iOvr++){
        for(int jRes=y, jOvr=0; jRes<imgRes.columns && jOvr<overlay.columns; jRes++, jOvr++){
            imgRes.data[iRes][jRes].red = alfa * overlay.data[iOvr][jOvr].red + ((1 - alfa)*imgRes.data[iRes][jRes].red);
            imgRes.data[iRes][jRes].green = alfa * overlay.data[iOvr][jOvr].green + ((1 - alfa)*imgRes.data[iRes][jRes].green);
            imgRes.data[iRes][jRes].blue = alfa * overlay.data[iOvr][jOvr].blue + ((1 - alfa)*imgRes.data[iRes][jRes].blue);
        }
    }
...
void ppm_overlayMain(int argc, char *argv[]){
    ...
    switch(argc){
        case 4:
            fileInput2=stdin;
            fileOutput=stdout;
            ppm_overlay(alfa ,createStruct(fileInput1), createStruct(fileInput2), fileOutput, charToInt(argv[1]),  charToInt(argv[2]));
            break;
        case 5:
